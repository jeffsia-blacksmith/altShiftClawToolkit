...
- **類型**：string
- **说明**：顯示名稱，使用英文，與目錄名對應但可加空格與大寫
- **規則**：
  - 將目錄名的 `-` 转為空格，每个單字首字母大寫
...
- **語言**：繁体中文
- **说明**：一句話簡介，不超過 50 字
- **範例**：`"多格式內容摘要工具，自動辨識输入類型並输出繁体中文摘要"`

### description（必填）

- **類型**：string
- **語言**：繁体中文
- **说明**：詳細描述，2-4 句話，说明功能、技術、適用情境
- **範例**：`"Gemini Summary 可自動偵測输入來源類型（網頁、PDF、影片、音訊），透過 Google Gemini API 產生繁体中文摘要。支援多種格式，一鍵取得精簡重點整理。"`
...
- **说明**：語意化版本號（SemVer）
- **規則**：
  - 優先从 `package.json` 的 `version` 欄位取得
...
- **類型**：string[]
- **说明**：該 skill/template 所需的環境变數名稱
- **規則**：
  - 無需環境变數時使用空陣列 `[]`
  - 環境变數名稱使用全大寫、底線分隔（如 `GEMINI_API_KEY`）
  - 提取來源優先順序：
    1. SKILL.md 的 `required_env` frontmatter 欄位
    2. README.md 中明確提及的環境变數
    3. scripts 中 `process.env.XXX` 或 `$XXX` 的用法
    4. .codex/config.toml 或 .copilot/config.json 中的環境变數設定
